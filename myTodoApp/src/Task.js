...
export default class Task extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.isEditing && !this.props.isEditing) {
          const task = this.props.tStore.findTask(newProps.id);
    ...
    changeDone = (event) => {
        ...
        this.props.tStore.changeDone(checked, this.props.id);
      };
        
    handleDeleteTaskClick = () => {
        if (window.confirm('The Task will be permanent delete')) 
        this.props.tStore.deleteTask(this.props.id)
        };
    ...
    handleSaveClick = () => {
        this.props.tStore.updateTask(this.props.id , this.state.taskText , this.state.taskDate.format('YYYY-MM-DD'));
    ...
    render() {
        ...
        const task = this.props.tStore.findTask(id);
